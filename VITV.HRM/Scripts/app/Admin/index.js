...
indexUserModule.factory("IndexUserService", ["$http", function ($http) {
    return {
        GetList: function () {
            return $http.get("/Manage/GetUsers").then(function (result) {
                return result.data;
            });
        }
    };
...
indexUserModule.controller("IndexUserModuleCtrl", ["$scope", "$http", "$timeout", "IndexUserService", function ($scope, $http, $timeout, IndexUserService) {
    ...
    $scope.ChangePassword = function (User) {
        window.location.href = "/Manage/ChangePassword?userId=" + User.Id;
    };

    $scope.deleteUser = function (User) {
        window.location.href = "/Manage/Delete/" + User.Id;
    };
